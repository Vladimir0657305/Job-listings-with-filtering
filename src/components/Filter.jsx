...
export default function Filter() {
    ...
    //         if (out.includes(uniq[j])) arrId.push(i);
    //     }
    // }
    for (const key of Object.keys(dataArr)) {
        ...
        for (let j = 0; j < uniq.length; j++) {
            // console.log('OUT', out, uniq[j], out.includes(uniq[j]));
            if (out.includes(uniq[j])) arrId.push(dataArr[key].id);
        }
    }
    // console.log(dataArr);
    // console.log(arrId);

    let newDataArr = dataArr.filter(obj => { return arrId.includes(obj.id) })
    ...
    return (
        <>
            <div className='card-body filter-block'>
                {
                    uniq.map(item => <span key={item + 10}> <button className='buttonStyleFilter' key={item}>{item} {svg}</button> </span>)
                }
            </div>
            {
